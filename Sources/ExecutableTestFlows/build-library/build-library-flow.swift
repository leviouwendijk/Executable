        ...
        TestFlow(
            ...
            Step("preserve scoped distribution build arguments") {
                ...
                try Expect.equal(
                    ...
                    "distribution build is scoped to the library target"
                )
            }
            ...
            Step("build library distribution artifacts end to end") {
                ...
                let expectedArtifactsDir = modulesRoot
                    .appendingPathComponent(
                        "ExecutableFixture",
                        isDirectory: true
                    )
                ...
                try Expect.equal(
                    output.artifactsDir.standardizedFileURL.path,
                    expectedArtifactsDir.standardizedFileURL.path,
                    "non-local build returns exported artifact directory"
                )

                let exportedInterface = output.artifactsDir
                    .appendingPathComponent(
                        "FixtureCore.swiftinterface"
                    )

                try Expect.true(
                    FileManager.default.fileExists(
                        atPath: exportedInterface.path
                    ),
                    "export includes FixtureCore.swiftinterface"
                )

                let exportedInterfaceText = try String(
                    contentsOf: exportedInterface,
                    encoding: .utf8
                )

                try Expect.equal(
                    exportedInterfaceText,
                    interface,
                    "exported interface preserves emitted interface contents"
                )
